...
def setup_gui(app):
    ...
    with dpg.window(
        ...
        with dpg.tab_bar():
            ...
            def _build_detection_tab():
                with dpg.tab(label="Detection"):
                    dpg.add_spacer(height=5)
                    dpg.add_text("COLOR SENSE", color=(201, 0, 141))
                    with dpg.group(horizontal=True):
                        ...
                        def on_color_slider_changed():
                            r = max(0, min(255, int(dpg.get_value(app.color_r))))
                            g = max(0, min(255, int(dpg.get_value(app.color_g))))
                            b = max(0, min(255, int(dpg.get_value(app.color_b))))
                            new_hex = (r << 16) | (g << 8) | b
                            app.config.update("target_color", new_hex)

                            # Sync UI state
                            if dpg.does_item_exist(app.color_display_item):
                                dpg.configure_item(app.color_display_item, value=[r, g, b, 255])
                        ...
                        with dpg.group(horizontal=True):
                            ...
                            with dpg.group(width=160):
                                app.color_r = dpg.add_slider_int(
                                    ...
                                    max_value=255,
                                    width=150,
                                    callback=on_color_slider_changed,
                                )
                            ...
                            with dpg.group():
                                dpg.add_spacer(height=20)
                                app.color_display = dpg.add_button(label="", width=50, height=50)

                        with dpg.theme() as c_theme:
                            with dpg.theme_component(dpg.mvButton):
                                app.color_display_item = dpg.add_theme_color(
                                    dpg.mvThemeCol_Button, [init_r, init_g, init_b, 255], category=dpg.mvThemeCat_Core
                                )
                                dpg.add_theme_color(
                                    dpg.mvThemeCol_ButtonHovered,
                                    [init_r, init_g, init_b, 255],
                        ...
                        dpg.bind_item_theme(app.color_display, c_theme)

                    dpg.add_spacer(height=5)
                    dpg.add_text("Color Tolerance:")
                    with dpg.group(horizontal=True):
                        app.tolerance_slider = dpg.add_slider_int(
                            ...
                            width=250,
                            callback=lambda s, a: [
                                v := max(0, min(100, int(a))),
                                dpg.set_value(s, v),
                                app.config.update("color_tolerance", v),
                                app.update_tolerance_preview(),
                            ][-1],
                        )
                        ...
                        app.tolerance_theme = t_theme
                        dpg.bind_item_theme(app.tolerance_preview, t_theme)
                    app.update_tolerance_preview()
                    ...
                    dpg.add_text("FOV SETTINGS", color=(201, 0, 141))
                    with dpg.group(horizontal=True):
                        dpg.add_text("Width:")
                    ...
                    app.fov_overlay_checkbox = dpg.add_checkbox(
                        label="Show Visual FOV Overlay", default_value=False, callback=on_fov_overlay_toggled
                    )
            ...
            def _build_system_tab():
                with dpg.tab(label="System"):
                    ...
                    def refresh_ui_from_config():
                        ...
                        if dpg.does_item_exist(app.color_r):
                            c = app.config.target_color
                            r, g, b = (c >> 16 & 0xFF), (c >> 8 & 0xFF), (c & 0xFF)
                            dpg.set_value(app.color_r, r)
                            dpg.set_value(app.color_g, g)
                            dpg.set_value(app.color_b, b)
                            if hasattr(app, "color_display_item") and dpg.does_item_exist(app.color_display_item):
                                dpg.configure_item(app.color_display_item, value=[r, g, b, 255])
                    ...
                    with dpg.theme() as reset_theme:
                        with dpg.theme_component(dpg.mvButton):
                            ...
                            dpg.add_theme_color(
                                dpg.mvThemeCol_ButtonHovered, (150, 50, 50), category=dpg.mvThemeCat_Core
                            )
                            dpg.add_theme_color(
                                dpg.mvThemeCol_ButtonActive, (180, 60, 60), category=dpg.mvThemeCat_Core
                            )
                ...
                if hasattr(app, "logger") and app.logger.debug_console_enabled:
                    with dpg.tab(label="Debug"):
                        ...
                        dpg.add_button(
                            ...
                            callback=lambda: app.toggle_debug_console()
                            if hasattr(app, "toggle_debug_console")
                            else None,
                            width=-1,
                        )
