...
# Protocol for user32 interface to satisfy type checkers
class User32Protocol(Protocol):
    def GetSystemMetrics(self, index: int) -> int: ...
    def GetCursorPos(self, point_ref: ctypes.c_void_p) -> int: ...
    def SendInput(self, nInputs: int, pInputs: ctypes.c_void_p, cbSize: int) -> int: ...
...
class LowLevelMovementSystem:
    ...
    def __init__(self, config: Any) -> None:
        ...
        # Cache user32 interface immediately to avoid repeated lookup
    ...
    def _resolve_user32(self) -> User32Protocol | None:
        """Helper to resolve the correct user32 instance (real or mocked) once."""
    ...
    def _get_user32(self) -> User32Protocol | None:
        """
        Legacy helper for compatibility, now returning cached instance.
        Maintains API for tests that might patch this method.
        """
        return self._user32
    ...
    def move_mouse_relative(self, dx: int, dy: int) -> bool:
        ...
        try:
            # Cast to c_void_p to satisfy Pyright and Protocol
            p_input = ctypes.cast(ctypes.byref(self._input_structure), ctypes.c_void_p)
            result = self._user32.SendInput(1, p_input, ctypes.sizeof(INPUT))
            return result == 1
        except Exception:
            return False
