...
- Pre-allocates ctypes structures to eliminate object creation overhead in the hot path.
- Caches user32 interface to avoid repeated library lookups.
- Uses threading.Lock to ensure thread-safety of the shared Input structure.
"""
...
class LowLevelMovementSystem:
    ...
    - Pre-allocated `INPUT` structure (O(1) allocation).
    - Thread-safe `threading.Lock` wrapper around the shared structure.
    - Cached `user32` handle.
    """
    ...
    def __init__(self, config: Any) -> None:
        ...
        # Thread safety for the shared ctypes structure
        ...
        # Cache user32 to avoid repeated lookup overhead
        self._user32 = self._get_user32()

        # Pre-allocate reusable ctypes structures for zero-allocation movement
        ...
        self._input_structure.ii.mi.dwExtraInfo = None  # type: ignore
    ...
    def move_mouse_relative(self, dx: int, dy: int) -> bool:
        ...
        Uses pre-allocated structure to prevent GC pressure.
        """
        if not self._user32:
            return True

        with self._lock:
            # Update the shared structure in-place
            mi = self._input_structure.ii.mi
            mi.dx = dx
            mi.dy = dy
            mi.dwFlags = MOUSEEVENTF_MOVE
            # dwExtraInfo is already None
