...
class LowLevelMovementSystem:
    ...
    def __init__(self, config: Any, perf_monitor: Any) -> None:
        ...
        # ULTRATHINK: Pre-calculate scaling factors to replace division in hot path
        # Clamp denominator to avoid zero division
        safe_w = max(2, self.screen_width)
        safe_h = max(2, self.screen_height)
        self._x_scale = 65535.0 / (safe_w - 1)
        self._y_scale = 65535.0 / (safe_h - 1)
    ...
    def move_mouse_absolute(self, x: int, y: int) -> bool:
        ...
        try:
            ...
            # OPTIMIZATION: Use pre-calculated scales and fast int conversion
