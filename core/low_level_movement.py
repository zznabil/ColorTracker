...
# ULTRATHINK: Type Protocol for User32 Interface
class User32Protocol(Protocol):
    """Protocol defining the expected interface for user32.dll interactions"""

    def GetSystemMetrics(self, index: int) -> int: ...
    def GetCursorPos(self, point: ctypes._Pointer) -> int: ...
    def SendInput(self, nInputs: int, pInputs: ctypes._Pointer, cbSize: int) -> int: ...
...
class LowLevelMovementSystem:
    ...
    def __init__(self, config: Any) -> None:
        ...
        # OPTIMIZATION: Cache User32 instance to eliminate repetitive getattr overhead
        # This provides ~5x speedup for interface resolution in hot paths
    ...
    def _resolve_user32(self) -> User32Protocol | None:
        """Helper to get the correct user32 instance (real or mocked) once at init"""
    ...
    def _get_user32(self) -> User32Protocol | None:
        """
        Legacy getter - kept for interface compatibility if needed,
        but internal usage should prefer self._user32 directly.
        """
        if self._user32 is None:
            # Fallback retry in case it was loaded late (unlikely but safe)
            self._user32 = self._resolve_user32()
        return self._user32
    ...
    def move_mouse_relative(self, dx: int, dy: int) -> bool:
        ...
        try:
            # Type cast used to silence Pyright about _CArgObject compatibility with Protocol
            result = self._user32.SendInput(1, ctypes.cast(ctypes.byref(self._input_structure), ctypes.c_void_p), ctypes.sizeof(INPUT))  # type: ignore
            return result == 1
        except Exception:
            return False
