...
OPTIMIZATIONS:
- Resolved `user32` interface in `__init__` to avoid repeated attribute lookups.
- Cached `user32` instance to ensure O(1) access in hot path.
"""
...
class LowLevelMovementSystem:
    ...
    def __init__(self, config: Any) -> None:
        ...
        # OPTIMIZATION: Resolve user32 interface once during initialization
        ...
        # Try to get actual metrics using the resolved user32
        if self._user32:
            try:
    ...
    def _resolve_user32(self) -> Any | None:
        ...
        This avoids repeated lookups and hasattr checks in the hot path.
        """
    ...
    def move_mouse_relative(self, dx: int, dy: int) -> bool:
        ...
        try:
            result = self._user32.SendInput(1, ctypes.byref(input_struct), ctypes.sizeof(INPUT))
            return result == 1
        except Exception:
            return False
