...
class DetectionSystem:
    ...
    def _scan_region(
        ...
        """
        Unified scan logic for a specific screen region.
        Extracts common logic from local and full search.
        """
        width = right - left
        height = bottom - top
        ...
        try:
            # We perform color matching directly on BGRA data (Optimization).
            mask = cv2.inRange(img_bgra, self._lower_bound, self._upper_bound)  # type: ignore
            # Optimization: Use minMaxLoc instead of findNonZero for O(1) memory
        ...
        return True, max_loc[0] + left, max_loc[1] + top
    ...
    def _local_search(self) -> tuple[bool, int, int]:
        ...
        # Unified scan call
        success, screen_x, screen_y = self._scan_region(
