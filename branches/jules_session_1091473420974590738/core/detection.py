...
class DetectionSystem:
    ...
    def _scan_region(
        self, left: int, top: int, right: int, bottom: int, max_clamp_size: int = 1500
    ) -> tuple[bool, int, int]:
        """
        Common scanning logic for both local and full search.
        Encapsulates clamping, capture, and detection.
        """
        width = right - left
        height = bottom - top
        ...
        # Optimization: Clamp search area if it exceeds max safe size
        left, right, top, bottom = self._clamp_search_area(
            left, right, top, bottom, max_size=max_clamp_size
        )
    ...
    def _local_search(self) -> tuple[bool, int, int]:
        ...
        # We pass max_clamp_size=500 to match original intent of clamping excessively large local areas,
        # although with search_area=100, this is technically unreachable.
        found, screen_x, screen_y = self._scan_region(
            local_left, local_top, local_right, local_bottom, max_clamp_size=500
        )
        ...
        if (
            abs(screen_x - self._center_x) > self._fov_x
            or abs(screen_y - self._center_y) > self._fov_y
        ):
            self.target_found_last_frame = False
            return False, 0, 0
    ...
    def _full_search(
        ...
        """
        found, screen_x, screen_y = self._scan_region(
            left, top, right, bottom, max_clamp_size=1500
        )
