        ...
        def logic_loop():
            try:
                ...
                for i in range(500):
                    ...
                    # Capture fov values for this iteration (Thread Safe Copy)
                    curr_fov_x = config.fov_x
                    curr_fov_y = config.fov_y

                    # Mock SCT for detection
                    with patch.object(ds, "_get_sct") as mock_sct:
                        img = np.zeros((curr_fov_y, curr_fov_x, 4), dtype=np.uint8)
                        # Target moving in circle
                        tx = (curr_fov_x // 2) + int(20 * np.cos(i * 0.1))
                        ty = (curr_fov_y // 2) + int(20 * np.sin(i * 0.1))
                        # Bound checks
                        tx = max(0, min(curr_fov_x - 1, tx))
                        ty = max(0, min(curr_fov_y - 1, ty))
