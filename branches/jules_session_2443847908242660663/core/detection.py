...
class DetectionSystem:
    ...
    def _detect_in_area(self, left: int, top: int, width: int, height: int) -> tuple[bool, bool, int, int]:
        """
        Helper method to perform detection within a specific area.

        Args:
            left, top, width, height: Area dimensions.

        Returns:
            (capture_success, found, screen_x, screen_y)
        """
        ...
        return True, True, int(max_loc[0] + left), int(max_loc[1] + top)
    ...
    def _local_search(self) -> tuple[bool, int, int]:
        ...
        capture_success, found, screen_x, screen_y = self._detect_in_area(local_left, local_top, width, height)
    ...
    def _full_search(self, left: int, top: int, right: int, bottom: int) -> tuple[bool, int, int]:
        ...
        capture_success, found, screen_x, screen_y = self._detect_in_area(left, top, width, height)
